<template>
    <div class='face' :class="{ translatePerspective: pointPassed }" :style="{background: $vuetify.theme.themes[theme].background}">
      ...
      <template v-slot:activator="{ on, attrs }">
        ...
        <v-icon
          :class="{ icon_down: true, stopAnimation: pointPassed}">
            mdi-chevron-down
          </v-icon>
      </template>
    ...
    computed: {
      rotation() {
        if (this.rotationMap[this.title] != 0) {
          return {
            transform: 'rotateX(' + this.rotationMap[this.title] + 'deg) scale3d(0.5,0.5,0.5) translateX(var(--translate-header)'
          };
        }
        return null;     
      },
